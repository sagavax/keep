...
while ($row = mysqli_fetch_array($result)) {
	...
	echo "<div class='keep-header'><span>" . $keep_title . "</span><div class='del'><a href='#' onclick='remove_note($note_id)'><i class='fa fa-times'></i></a><a href='#' onclick='unpine_note($note_id)' title='unpin note'><i class='fa fa-thumbtack'></i></a></div></div>";
...
while ($row = mysqli_fetch_array($result)) {
	...
	echo "<div class='keep-note' note-id='$note_id'>";
	echo "<div class='keep-header'><span>" . $keep_title . "</span><div class='del'><a href='#' onclick='remove_note($note_id)'><i class='fa fa-times'></i></a><a href='#' onclick='pine_note($note_id)' title='Pin note'><i class='fa fa-thumbtack'></i></a></div></div>";
	echo "<article>" . makeLinks(nl2br($keep_text)) . "</article>
                           		<footer>$date</footer>
					        </div>"; //zobraz ich ako FB wall
