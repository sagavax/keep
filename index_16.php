...
while ($row = mysqli_fetch_array($result)) {
	...
	echo "<div class='keep-header'><span>" . $keep_title . "</span><div class='note_action'><button class='pine_note' onclick='remove_note($note_id)'><i class='fa fa-times'></i></button><button class='pine_note' type='button' onclick='unpine_note($note_id)' title='unpin note'><i class='fa fa-thumbtack'></i></button></div></div>";
...
?>
		...
		<div id="keep-notes">
				...
				<?php
//$sql = "SELECT *,TIME(date_updated) as time,DATE(date_updated) as date  from keep WHERE is_pinned = 0 ORDER BY date_updated DESC";
...
while ($row = mysqli_fetch_array($result)) {
	...
	echo "<div class='keep-note' note-id='$note_id'>";
	echo "<div class='keep-header'><span>" . $keep_title . "</span><div class='note_action'><button class='pine_note' type='button' onclick='remove_note($note_id)'><i class='fa fa-times'></i></button><button type='button' onclick='pine_note($note_id)' class='pine_note' title='Pin note'><i class='fa fa-thumbtack'></i></button></div></div>";
	echo "<article>" . makeLinks(nl2br(trim($keep_text))) . "</article>
                           		<footer>$date</footer>
					        </div>"; //zobraz ich ako FB wall
